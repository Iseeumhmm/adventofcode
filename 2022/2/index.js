const fs = require('fs');
const roundsArr = fs.readFileSync('input.txt').toString().split("\n").map(r=>r.split(/\s+/g));
...
const newTotal = roundsArr.map(r=>r[1] == "X" ? Number(loose[r[0]]) : r[1] == "Y" ? Number(trnslt[r[0]]) + 3 : r[1] == "Z" ? Number(win[r[0]]) + 6 : null).reduce((a,b)=>a+b, 0);
